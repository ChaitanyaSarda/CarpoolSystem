<?php
	...
	if(!empty($_SESSION['user_id'])){
			...
			$q = "select `request_table_id`,`sharingtable_id` from `REQUESTMESSAGE` where `sharer_id` = '$uid' and `isread` = '0'";
			$qr = mysql_query($q);
			$q_count = mysql_num_rows($qr);
			echo '<div id="intro2"><p style="font-size:15px;">Requests:</p><br>';
			for($i = 0;$i<$q_count;$i++){
				...
				echo $i.') Your request for sharing SOURCE:"'.$source.'", DEST:"'.$dest.'", DAY:"'.$day.'" has been <span style="color:red">REJECTED</span><br><br>';
			}
			if($q_count == 0)
				echo 'No requests denied';
			echo '</div>';
			$q = "select `request_table_id` from `REQUESTMESSAGE` where `requester_id` = '$uid' and `isread` = '1' and `accepted` = '1'";
			$qr = mysql_query($q);
			$q_count = mysql_num_rows($qr);
			echo '<div id="r2"><p style="font-size:15px;">Requests Accepted:</p><br>';
			for($i = 0;$i<$q_count;$i++){
				$rid = mysql_result($qr,$i,'request_table_id');
				$q1 = "select * from `REQUEST_TABLE` where `rid` = '$rid'";
				$qr1 = mysql_query($q1);
				...
				$i++;
				if($is_daily == 1)$day = "Daily";else $day = $date;
				echo $i.') Your request for sharing SOURCE:"'.$source.'", DEST:"'.$dest.'", DAY:"'.$day.'" has been <span style="color:green">ACCEPTED</span><br><br>';
			}
			if($q_count == 0)
				echo 'No requests accepted';
			echo '</div>';
			$q = "select `request_table_id` from `REQUESTMESSAGE` where `requester_id` = '$uid' and `isread` = '0'";
			$qr = mysql_query($q);
			$q_count = mysql_num_rows($qr);
			echo '<div id="r3"><p style="font-size:15px;">Unread Requests:</p><br>';
			for($i = 0;$i<$q_count;){
				$rid = mysql_result($qr,$i,'request_table_id');
				$q1 = "select * from `REQUEST_TABLE` where `rid` = '$rid'";
				$qr1 = mysql_query($q1);
				...
				$i++;
				if($is_daily == 1)$day = "Daily";else $day = $date;
				echo $i.') Your request for sharing SOURCE:"'.$source.'", DEST:"'.$dest.'", DAY:"'.$day.'" has not been read<br><br>';
			}
			if($q_count == 0)
...
<style type="text/css">
	*	{margin:0px;padding:0px;}
	#wrapper {margin-left:auto;margin-right:auto;width:70%;margin-top:auto;margin-bottom:auto;height:650px;border:5px solid blue;}
